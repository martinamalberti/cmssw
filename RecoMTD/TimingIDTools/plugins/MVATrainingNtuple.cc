...
private:
  ...
  std::vector<bool> Ttrack_Signal, Ttrack_Associated, Ttrack_TPDirClu, Ttrack_TPOtherClu,
      Ttrack_isBTL, Ttrack_isETL, Ttrack_withMTD;
      ...
      if (trkRecSel(trackGen)) {
        ...
        if (tp_info != nullptr && trkTPSelAll(**tp_info)) {
          ...
          bool isTPmtdDirectBTL = false, isTPmtdOtherBTL = false,
              isTPmtdDirectETL = false, isTPmtdOtherETL = false,
              isTPmtdDirectCorrectBTL = false, isTPmtdOtherCorrectBTL = false,
              isTPmtdDirectCorrectETLD1 = false, isTPmtdOtherCorrectETLD1 = false,
              isTPmtdDirectCorrectETLD2 = false, isTPmtdOtherCorrectETLD2 = false;
                  ...
                  for (const auto& simClusterRef_RecoMatch : simClustersRefs_RecoMatch) {
                    ...
                    if (simClusterIt != simClustersRefs.end()) {
                      ...
                      if (isETL) {
                        ...
                        if (detid.nDisc() == 1) {
                          isTPmtdDirectCorrectETLD1 = true;
                        } else if (simClusterRef_RecoMatch->trackIdOffset() != 0) {
                          isTPmtdOtherCorrectETLD1 = true;
                        }
                        if (detid.nDisc() == 2) {
                          isTPmtdDirectCorrectETLD2 = true;
                        } else if (simClusterRef_RecoMatch->trackIdOffset() != 0) {
                          isTPmtdOtherCorrectETLD2 = true;
                        }
                      }
                    }
                  }
          ...
          if (isTPmtdDirectCorrectBTL || isTPmtdOtherCorrectBTL ||
              isTPmtdDirectCorrectETLD1 || isTPmtdOtherCorrectETLD1 ||
              isTPmtdDirectCorrectETLD2 || isTPmtdOtherCorrectETLD2) {
          ...
          if (isTPmtdOtherBTL || isTPmtdOtherETL) {
            isTPDirClu = false;  // Not direct cluster
            isTPOtherClu = true;
          }
...
const bool MVATrainingNtuple::trkTPSelAll(const TrackingParticle& tp) {
  ...
  match = tp.charge() != 0 && std::abs(tp.eta()) < etacutGEN_ &&
	  ((std::abs(tp.eta()) < etacutBTL_ && tp.pt() > pTcutBTL_) || (std::abs(tp.eta()) > etacutBTL_ && tp.pt() > pTcutETL_)) &&
...
const bool MVATrainingNtuple::trkRecSel(const reco::TrackBase& trk) {
  bool match = false;
  match = std::abs(trk.eta()) <= etacutREC_ && ( (std::abs(trk.eta()) <= etacutBTL_ && trk.pt() > pTcutBTL_ ) ||  (std::abs(trk.eta()) > etacutBTL_ && trk.pt() > pTcutETL_) ) ;
  return match;
}
