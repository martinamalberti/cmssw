...
namespace ALPAKA_ACCELERATOR_NAMESPACE {
  ...
  class BTLRawToDigi
      : public stream::
            EDProducer<> {  // stream::EDProdicer vedi: https://twiki.cern.ch/twiki/bin/view/CMSPublic/FWMultithreadedFrameworkModuleTypes?utm_source=chatgpt.com#Comparing_Stream_and_Global_Modu
  public:
  ...
  void BTLRawToDigi::produce(device::Event& iEvent, device::EventSetup const& iSetup) {
    ...
    int32_t nChannels = 0;
    for (uint32_t fed = BTLElectronicsSpecs::kFirstFEDId;
         fed < BTLElectronicsSpecs::kFirstFEDId + BTLElectronicsSpecs::kNumberOfFEDs;
         ++fed) {
